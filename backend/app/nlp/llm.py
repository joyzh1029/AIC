...
async def generate_response(
    ...
    if _gemini_model is None:
        logger.error(
            "Gemini model is not initialized. Attempting to initialize now (this is not ideal)."
        )
        try:
            configure_gemini()
            ...
            logger.error(
                f"Critical error: Gemini model not available for response generation: {e}",
                exc_info=True,
            )
    ...
    if ai_mbti_persona:
        mbti_description = MBTI_PERSONAS.get(
            ai_mbti_persona, f"너는 {ai_mbti_persona} 성격의 AI야."
        )
    ...
    logger.info(
        f"Generated prompt for Gemini: {prompt}"
    )  # ✨ 생성된 프롬프트 로깅 (핵심 1)
    ...
    try:
        ...
        # ✨ Gemini 응답 객체 확인 로깅
        logger.info(
            f"Gemini raw response object type: {type(response)}"
        )  # 응답 객체 타입
        logger.info(
            f"Gemini raw response parts: {response.parts if hasattr(response, 'parts') else 'N/A'}"
        )  # 응답 parts
        logger.info(
            f"Gemini raw response text: {response.text.strip()}"
        )  # ✨ 실제 텍스트 응답 (핵심 2)
